...
def find_instance(items, class_or_tuple):
    for item in items:
        if isinstance(item, class_or_tuple):
            return item
    return None 
@ThorfinRobot(pattern="^/searchsticker (.*)")
async def _(event):
...
async def _(event):
    ...
    packshortname = f"Thorfinn_stickers_{userid}"
    ...
    if is_a_s:
        ...
        packshortname = f"Thorfinn_animated_{userid}"
